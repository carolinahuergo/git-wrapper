...
async def get_commits_for_all_branches(service: CommitsService = Depends()): 
...
async def get_commits_by_branch(branch_name: str, service: CommitsService = Depends()):
...
async def get_branch_commit_by_hexsha(branch_name: str, hexsha: str, service: CommitsService = Depends()):
    return service.get_branch_commit_by_hexsha(branch_name, hexsha)

@router.post("/", status_code = status.HTTP_201_CREATED)
async def commit(commit_input: CommitInput, service: CommitsService = Depends()):
